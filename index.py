...
class Indexer:
    ...
    def process_document(self, title: str, id: int, body: str) -> list[str]:
        ...
        """
        body_words = re.findall(self.tokenization_regex, body)
        title_words = re.findall(self.tokenization_regex, title)
        ...
        for word in words:
            ...
            # extract the link text 
                link_text, link_dest = self.split_link(word)

                if link_dest in self.titles_to_ids and link_dest is not self.ids_to_titles[id]: 
                    if id not in self.ids_to_links:
                        self.ids_to_links[id] = set()
                    link_id = self.titles_to_ids[link_dest] 
                    self.ids_to_links[id].add(link_id)

                for link_word in link_text:
                    processed_link_word = self.stem_and_stop(link_word)
                    if processed_link_word:
                        processed_words.append(processed_link_word)
            else:
                processed_word = self.stem_and_stop(word)
                # if not none 
                if processed_word:
                    processed_words.append(processed_word)
        
        self.compute_frequency(id, processed_words)
        return processed_words
        
    def compute_frequency(self, id: int, list: list[str]):
        ...
        for word in list:
            if word not in self.words_to_doc_frequency:
                ...
                if id not in self.words_to_doc_frequency[word]:
                    ...
                    self.words_to_doc_frequency[word][id] += 1
            if self.words_to_doc_frequency[word][id] > max_count:
                max_count = self.words_to_doc_frequency[word][id]

        if max_count > self.ids_to_max_counts.get(id):
            self.ids_to_max_counts[id] = max_count
    ...
    def parse(self):
        ...
        for wiki_page in wiki_xml_root: 
            ...
            id = self.titles_to_ids[title]
            text = wiki_page.find("text").text.strip()
            if text is None: 
                text = ""
            self.process_document(title, id, text)
    ...
    def compute_tf(self) -> dict[str, dict[int, float]]:
        ...
        for word in self.words_to_doc_frequency:
            ...
            for doc in self.words_to_doc_frequency[word]:
                tf_dict[word][doc] = 0
                if doc in self.ids_to_max_counts:
                    tf_dict[word][doc] = float(self.words_to_doc_frequency[word][doc]/float(self.ids_to_max_counts[doc]))
        return tf_dict
    ...
    def compute_idf(self) -> dict[str, float]:
        ...
        idf_dict = {} 
        for word in self.words_to_doc_frequency: 
            idf_dict[word] = math.log(len(self.ids_to_titles)/len(self.words_to_doc_frequency[word]))
        return idf_dict
    ...
    def compute_term_relevance(self) -> dict[str, dict[int, float]]:
        ...
        for word in tf_dict:
            term_relevance[word] = {}
            for doc in tf_dict[word]:
                term_relevance[word][doc] = float(tf_dict[word][doc] * idf_dict[word])
    ...
    def compute_page_rank(self) -> dict[int, float]:
        ...
        for id in self.ids_to_titles:
            ranks[id] = 0
            next_ranks[id] = 1/len(self.ids_to_titles)

        while self.distance(ranks, next_ranks) > self.DISTANCE_THRESHOLD:
            ranks = next_ranks.copy()

            for j in self.ids_to_titles:
                next_ranks[j] = 0.0
                next_ranks[j] = next_ranks[j] + sum(weights[k][j] * ranks[k] for k in self.ids_to_titles)
